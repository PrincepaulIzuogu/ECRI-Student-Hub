...
const DashboardPage = () => {
  ...
  useEffect(() => {
    filterMentees();
  }, [mentees, searchTerm, selectAll]);

  const fetchMentees = async (email) => {
    try {
      setIsLoading(true);
      const response = await axios.post(
        'http://127.0.0.1:8000/api/getMentees',
        { email }
      );
      ...
      <table className="mentees-table">
        ...
        <tbody>
          {isLoading ? (
            ...
            <tr>
              <td colSpan="6">No mentees found.</td>
            </tr>
          ) : (
    ...
    <div className="dashboard-page">
      <div className="top-nav">
        <div className="welcome-message">
          <h2>
            Mentor's Dashboard for {degree} {course} {semester}
          </h2>
        </div>
    ...
    <button className="search-button" onClick={filterMentees}>
      Search
          ...
          <div className="select-options">
            ...
            <button className="select-button">Select Mentee</button>
          </div>
